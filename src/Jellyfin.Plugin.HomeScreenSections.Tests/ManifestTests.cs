...
public class ManifestTests
{
    ...
    private static readonly Regex AbiRegex = new(@"^\d+\.\d+\.\d+\.\d+$", RegexOptions.Compiled);
    ...
    public void Manifest_versions_have_valid_fields()
    {
        ...
        foreach (JToken token in versions)
        {
            ...
            Assert.StartsWith("https://", sourceUrl);
            Assert.EndsWith(".zip", sourceUrl);

            JArray? deps = version["dependencies"] as JArray;
            Assert.NotNull(deps);
            Assert.Contains(deps!, d => d.Value<string>() == "5e87cc92-571a-4d8d-8d98-d2d4147f9f90");
        }
    }
    ...
    public void Manifest_includes_package_for_jellyfin_10_11_11()
    {
        ...
        bool has = versions.Any(v =>
            v.Value<string>("targetAbi") == "10.11.11.0" &&
